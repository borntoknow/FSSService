            ...
            if (endpointBehavior is SickList.EndpointBehaviorSickList)
            {
                switch (messageSignature)
                {
                    case "http://www.fss.ru/integration/ws/eln/ins/getPrivateLNData/v01":
                        fssMessage = new MessageSickListGet(message, endpointBehavior as SickList.EndpointBehaviorSickList);
                        break;
                    case "http://www.fss.ru/integration/ws/eln/ins/prParseReestrFile/v01":
                        fssMessage = new MessageSickListSet(message, endpointBehavior as SickList.EndpointBehaviorSickList);
                        break;
                    default:
                        fssMessage = new MessageSickList(message, endpointBehavior as SickList.EndpointBehaviorSickList);
                        break;
                }
            }
